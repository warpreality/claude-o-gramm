...
class SessionManager:
    ...
    async def send(self, key: str, text: str) -> None:
        ...
        if not await tmux.has_session(live.rec.tmux):
            ...
            return
        await tmux.type_text(live.rec.tmux, text)
        await asyncio.sleep(0.3)
        await tmux.send_keys(live.rec.tmux, "Enter")
    ...
    def _command(self, rec: SessionRec, resume: bool, prompt: str | None) -> str:
    ...
    async def _launch(self, live: Live, resume: bool, prompt: str | None) -> None:
        rec = live.rec
        live.starting.clear()
        # короткий промпт передаём аргументом (надёжно), длинный — вставкой после старта
        inline = prompt if prompt and len(prompt) < 3000 else None
        try:
            await tmux.new_session(rec.tmux, rec.cwd, self._command(rec, resume, inline))
            if not live.tail_task or live.tail_task.done():
                self._start_tail(live)
            await self._babysit_startup(live, paste=None if inline else prompt)
        finally:
            live.starting.set()
    ...
    async def _babysit_startup(self, live: Live, paste: str | None) -> None:
        ...
        if paste:
            ...
            await tmux.type_text(name, paste)
            await asyncio.sleep(0.3)
            await tmux.send_keys(name, "Enter")
    ...
    def _cancel(self, live: Live) -> None:
        for task in (live.tail_task, live.typing_task):
            if task:
                task.cancel()
